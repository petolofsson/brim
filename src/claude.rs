...
/// Compute window fill from the last-turn usage fields.
...
fn parse_last_turn(path: &Path) -> (Option<WindowInfo>, Option<DateTime<Utc>>) {
    ...
    for line in text.lines() {
        ...
        let model = msg
            ...
            .unwrap_or("")
            .to_string();
        result = Some(compute_window_info(input, cache_read, cache_create, &model));
    }
...
mod tests {
    ...
    fn test_window_tokens_no_overflow_huge_values() {
        let info = compute_window_info(u64::MAX, u64::MAX, u64::MAX, "claude-sonnet-4-6");
    ...
    fn test_window_fill_math_oracle() {
        let info = compute_window_info(7_000, 130_000, 5_000, "claude-sonnet-4-6");
    ...
    fn test_window_fill_bounded_at_100() {
        let info = compute_window_info(200_000, 100_000, 50_000, "claude-sonnet-4-6");
        assert_eq!(info.fill_percent, 100);
    }
    ...
    fn test_window_limit_1m_model() {
        ...
        let info = compute_window_info(500_000, 0, 0, "claude-opus-4-8[1m]");
    ...
    fn test_context_limit_stored() {
        let info = compute_window_info(100_000, 0, 0, "claude-sonnet-4-6");
        assert_eq!(info.context_limit, 200_000);
        let info_1m = compute_window_info(100_000, 0, 0, "claude-opus-4-8[1m]");
