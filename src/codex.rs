...
//! `input_tokens` (includes cached), `cached_input_tokens`.
//! Pure input = `input_tokens − cached_input_tokens`; that plus the cache
//! field feeds `compute_window_info`.
//!
...
/// `cache_creation_input_tokens` is not present in Codex TokenUsage and is omitted.
...
impl TokenCounts {
    fn all_zero(&self) -> bool {
        self.input_tokens == 0 && self.cached_input_tokens == 0
    }
}
...
fn extract_window(
    ...
    // window_tokens = pure_input + cache_read + 0 (no cache_create in Codex)
    //               = (input_tokens − cached) + cached
    //               = input_tokens
    let pure_input = counts
        .input_tokens
        .saturating_sub(counts.cached_input_tokens);
    let info = compute_window_info(pure_input, counts.cached_input_tokens, 0, model, source);
    ...
    let trend = {
        ...
        if tail_events.len() >= 2 {
            ...
            for i in 1..tail_events.len() {
                ...
                if d_in == 0 && d_cached == 0 {
                    continue;
                }
                let pure = d_in.saturating_sub(d_cached);
                let pt_info = compute_window_info(pure, d_cached, 0, model, WindowSource::LastTurn);
...
mod tests {
    ...
    // Turn 1 cumulative: input=100000, cached=80000.
    // Turn 2 cumulative: input=120000, cached=80000.
    // Delta: input_delta=20000, cached_delta=0.
    // pure_input=20000, window_tokens=20000+0=20000.
    #[test]
    fn test_codex_last_turn_delta_window() {
        let line1 = make_token_count_line(100_000, 80_000, "2024-06-01T10:00:00Z");
    ...
    // input=50000, cached=30000.
    // pure_input=20000, window=20000+30000=50000.
    #[test]
    ...
    fn test_codex_parse_session_file_round_trip() {
        ...
        // First turn: input=10000, cached=0.
        writeln!(
            f,
            "{}",
            make_token_count_line(10_000, 0, "2024-06-01T10:00:00Z")
        )
        .unwrap();
        // Second turn: input=23000, cached=8000.
        // Delta: d_in=13000, d_cached=8000; pure=5000; window=5000+8000=13000.
        writeln!(
            f,
            "{}",
            make_token_count_line(23_000, 8_000, "2024-06-01T10:01:00Z")
        )
        .unwrap();
        drop(f);
    ...
    fn test_codex_three_events_delta_of_last_two() {
        ...
        let line1 = make_token_count_line(1_000, 0, "2024-06-01T10:00:00Z");
        let line2 = make_token_count_line(5_000, 0, "2024-06-01T10:01:00Z");
    ...
    // ADR-006: Codex trend from per-turn occupancy deltas (targets 128k backstop).
    // Cumulative: 10k, 20k, 35k, 55k → deltas: 10k, 15k, 20k
    // compute_trend on [10k, 15k, 20k]: pos deltas = [5k, 5k] → velocity = 5k
    // projection = (128k - 20k) / 5k = 21
    #[test]
    fn test_codex_trend_rising_occupancy() {
        let line1 = make_token_count_line(10_000, 0, "2024-06-01T10:00:00Z");
        let line2 = make_token_count_line(20_000, 0, "2024-06-01T10:01:00Z");
        let line3 = make_token_count_line(35_000, 0, "2024-06-01T10:02:00Z");
        ...
        assert_eq!(t.projected_turns_to_recycle, Some(21));
    ...
    fn test_codex_trend_reset_detection() {
        let line1 = make_token_count_line(10_000, 0, "2024-06-01T10:00:00Z");
        let line2 = make_token_count_line(30_000, 0, "2024-06-01T10:01:00Z");
        let line3 = make_token_count_line(42_000, 0, "2024-06-01T10:02:00Z");
        let line4 = make_token_count_line(50_000, 0, "2024-06-01T10:03:00Z");
    ...
    fn test_codex_trend_dedupe_no_zero_point() {
        let line1 = make_token_count_line(10_000, 0, "2024-06-01T10:00:00Z");
        let line2 = make_token_count_line(10_000, 0, "2024-06-01T10:00:30Z"); // dup
        let line3 = make_token_count_line(20_000, 0, "2024-06-01T10:01:00Z");
        let line4 = make_token_count_line(20_000, 0, "2024-06-01T10:01:30Z"); // dup
    ...
    fn test_window_tokens_formula() {
        ...
        // pure_input = 100-60=40; window = 40 + 60 = 100 = input_tokens
        let pure_input = c.input_tokens.saturating_sub(c.cached_input_tokens);
        let wt = pure_input.saturating_add(c.cached_input_tokens);
        assert_eq!(wt, 100);
    }
}
