...
|---|---|---|---|---|---|---|---|
| **Claude Code** | `~/.claude/projects/<encoded-cwd>/<uuid>.jsonl`; JSONL one event per line; sub-agents at `<uuid>/subagents/agent-*.jsonl`. **Confirmed against real live sessions.** | Last `type:"assistant"` turn's `message.usage.{input_tokens, cache_read_input_tokens, cache_creation_input_tokens}`; point-in-time `window_tokens = input + cache_read + cache_create` (ADR-002). **Real-session PASS:** brim's `window_tokens` matched exactly on two live sessions (**293,858** and **70,370** tokens), `output_tokens` excluded. | Read (`cache_read_input_tokens`) and creation (`cache_creation_input_tokens`) both present. | Parent session UUID dir contains `subagents/agent-<agent-id>.jsonl`; brim joins by parent UUID → `agent_id` stem (code-confirmed). | Skipping zero-usage trailing turns; tail-read truncation risk on >256KB transcripts. | Docs-listed Claude models (Opus/Sonnet 200k base, `[1m]` variants 1M) via models.dev brim default 200k. Real model = **Pending**. | **VALIDATED — real session (occupancy oracle PASS).** Model-id/limit cell still docs-only. |
...
- **Real session:** captured. brim run against real live Claude Code transcripts confirmed the point-in-time last-turn window = `input + cache_read + cache_creation` of the last `assistant` turn, `output_tokens` excluded (ADR-002). Two sessions matched exactly: **293,858** and **70,370** tokens.
- **Discrepancy:** none for the occupancy oracle — docs, brim code, and real sessions now agree on the `subagents/agent-*.jsonl` layout and the last-turn `message.usage` field shape (`input_tokens`, `cache_read_input_tokens`, `cache_creation_input_tokens`). The real model id / per-model context limit remain docs-only (not asserted by the captured sessions).
...
- **ADR-002 (point-in-time window over cumulative aggregate) — confirmed.** Both OpenCode (step-finish `tokens`) and Codex (delta of cumulative `total_token_usage`) realize a point-in-time oracle; brim's design fits observed data. The Claude parity half is now **confirmed against real sessions** (two matched exactly: 293,858 / 70,370 tokens). Copilot is the lone exception (process-log oracle, already VERIFIED-LIVE).
...
- **Claude Code:** per-turn oracle = last `assistant` turn `message.usage`; ADR-006 trend supportable from the last K assistant turns. Real-session parity **confirmed** (two sessions matched exactly: 293,858 / 70,370 tokens).
...
- **Claude Code real-session capture** — **DONE.** Occupancy oracle validated against real live sessions (`window_tokens` matched exactly on two: 293,858 / 70,370 tokens; `output_tokens` excluded, ADR-002). Residual: the real model id and per-model context limit remain docs-only (not asserted by the captured sessions).
