...
| **codex** | Yes (live-verified, codex 0.142.2) | **Yes — WIRED** (live-verified codex 0.142.2, `codex.rs::extract_codex_behavior`; ADR-030) | **none usable** — real `function_call_output` has NO `status` field (keys: `call_id`,`output`,`type`); exit only in free-text `output` (r11 forbids parsing) → failure_streak CANNOT fire for real codex (ADR-031); legacy `status='failed'` path is dead spec-legacy | JSON **string** (hashed raw) | tool calls are DIRECT payload items (`payload.type` ∈ {function_call, function_call_output, custom_tool_call}), NOT content[]; occupancy from `payload.type='token_count'`. Repetition/ping-pong still fire; failure-streak dark |
| **opencode** | Yes (verified, 1.17.9) | **Yes — WIRED** (live-verified 1.17.9, `opencode.rs::extract_opencode_behavior`; ADR-028/ADR-031) | `state.status='error'` **OR** `state.metadata.exit` (i64) != 0 — LIVE-VERIFIED: failed bash keeps `status='completed'` but sets `metadata.exit` to the non-zero code (the real stuck symptom) | JSON **object** | tool rows in `part` (`data.type='tool'`); name `data.tool`, args `data.state.input`, last K=8 tail; stub removed, Behavior fires |
| **copilot** | **Yes** — `~/.copilot/session-state/<uuid>/events.jsonl` carries `tool.execution_start` (toolName, arguments) + `tool.execution_complete` (`success` bool) | **No — PENDING WIRING** (`copilot.rs` reads only the occupancy process-log, not `events.jsonl`) | `tool.execution_complete.success == false` (available, not yet parsed) | object (`arguments`) | NOT behavior-blind — tool structure + operation-failure signal present in `events.jsonl`; brim just doesn't parse it yet. Wiring is a separate upcoming unit (ADR-031 forward note) |
